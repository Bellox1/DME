...
class PatientController extends Controller
{
    ...
    //         // On utilise une fonction de rappel pour grouper les clauses OR
    //         $query->where(function ($mainQuery) use ($search) {
    //             $mainQuery->whereHas('utilisateur', function ($q) use ($search) {
    ...
    //                         ->orWhere('prenom', 'like', "%{$search}%");
    //                 })
    //                 // Vérifie bien que le nom de la colonne est 'id' ou 'numero_patient' dans ta DB
    //                 ->orWhere('id', 'like', "%{$search}%");
    //         });
    //     }

    //     // Important pour ton Frontend : paginate renvoie un objet avec une clé 'data'
    ...
    //             // Recherche dans les infos de l'utilisateur (Adulte)
    //             $q->whereHas('utilisateur', function ($sub) use ($search) {
    //                 $sub->where('nom', 'like', "%{$search}%")
    //                     ->orWhere('prenom', 'like', "%{$search}%");
    //             })
    //                 // OU dans les infos de l'enfant
    //                 ->orWhereHas('enfant', function ($sub) use ($search) {
    //                     $sub->where('nom', 'like', "%{$search}%")
    //                         ->orWhere('prenom', 'like', "%{$search}%");
    //                 })
    //                 // OU par l'ID du patient (puisque numero_patient n'existe pas)
    //                 ->orWhere('id', 'like', "%{$search}%");
    //         });
    //     }
    ...
    public function index(Request $request)
    {
        ...
        // On remet la pagination (pour le Frontend) 
        // MAIS on force le tri pour voir ISSOUMA Nafi en premier
        // return response()->json(
        //     $query->orderBy('created_at', 'desc')->paginate(20)
        // );

        // Remplace 'created_at' par 'date_creation'
        return response()->json(
    ...
    //     $tuteurId = $request->parent_id ?? Auth::user()->id;
    ...
    //         $userConnecte = Auth::user(); // Utilise Auth::user() pour la sécurité de session

    //         if ($request->type === 'soi_meme') {
    //             // Création dossier adulte
    //             $patient = Patient::create([
    //                 'utilisateur_id' => $userConnecte->id,
    //                 'enfant_id' => null,
    //                 'adresse' => $request->adresse,
    ...
    //             ]);
    //         } else {
    //             // Création Enfant sous TUTELLE
    //             $enfant = Enfant::create([
    //                 'parent_id' => $userConnecte->id, // Le lien de tutelle est ici
    //                 'nom' => $request->nom,
    //                 'prenom' => $request->prenom,
    //                 'sexe' => $request->sexe,
    //                 'date_naissance' => $request->date_naissance
    ...
    //         // TRACABILITÉ avec ton secret de session
    //         Tracabilite::create([
    //             'utilisateur_id' => $userConnecte->id,
    //             'action' => 'création_dossier',
    //             'nom_table' => 'patients',
    //             'nouvelle_valeur' => "Type: {$request->type} | Patient ID: {$patient->id}"
    //         ]);
    ...
    //                 'data' => [
    //                     'id' => $patient->id,
    //                     'numero_patient' => $patient->numero_patient, // Généré via un observer ou mutateur
    //                     'nom' => $user->nom,
    //                     'prenom' => $user->prenom,
    //                     'tel' => $user->tel
    //                 ]
    ...
    // {
    //     // On cherche le patient (soit par son ID, soit par l'ID utilisateur lié)
    //     $patient = Patient::where('id', $id)
    //         ->orWhere('utilisateur_id', $id)
    //         ->firstOrFail();

    //     // Si c'est un adulte (lié à un compte utilisateur)
    //     if ($patient->utilisateur_id) {
    //         $utilisateur = $patient->utilisateur;

    //         $validated = $request->validate([
    //             // Infos Identité (Table Utilisateurs)
    //             'nom' => 'sometimes|string|max:100',
    //             'prenom' => 'sometimes|string|max:100',
    //             // Le secret est ici : on ajoute ,'.$utilisateur->id pour ignorer son propre numéro
    //             'tel' => 'sometimes|string|unique:utilisateurs,tel,' . $utilisateur->id,
    //             'whatsapp' => 'nullable|string',
    //             'sexe' => 'sometimes|in:Homme,Femme,Masculin,Féminin',

    //             // Infos Médicales (Table Patients)
    ...
    //         try {
    //             // 1. Mise à jour de l'utilisateur (Nom, Tel, Sexe...)
    //             $utilisateur->update($request->only(['nom', 'prenom', 'tel', 'whatsapp', 'sexe']));

    //             // 2. Mise à jour du dossier patient (Ville, Taille, Poids...)
    //             $patient->update($request->only(['ville', 'adresse', 'taille', 'poids', 'groupe_sanguin', 'date_naissance']));

    //             return response()->json([
    //                 'status' => 'success',
    //                 'message' => 'Dossier complet mis à jour',
    //                 'data' => $patient->load('utilisateur') // On renvoie tout l'objet chargé
    //             ]);
    //         } catch (\Exception $e) {
    ...
    //     return response()->json(['error' => 'Type de patient non géré'], 400);
    // }
