...
const ReceptionLayout = ({ children }) => {
  ...
  const [user] = useState(() => {
    ...
    return saved
      ? JSON.parse(saved)
      : { nom: "Utilisateur", prenom: "", role: "accueil" };
  });

  const [isProfileOpen, setIsProfileOpen] = useState(false);

  useEffect(() => {
    localStorage.setItem("sidebar-expanded-reception", isSidebarOpen);
  }, [isSidebarOpen]);
  ...
  const menuSections = [
    {
      title: "Accueil",
      items: [
        { path: "/accueil", icon: "dashboard", label: "Tableau de bord" },
        {
          path: "/accueil/patients",
          icon: "person_search",
          label: "Liste Patients",
        },
        {
          path: "/accueil/enregistrement",
          icon: "person_add",
          label: "Inscription",
        },
      ],
    },
    {
      ...
      items: [
        ...
        {
          path: "/accueil/demandesrdv",
          icon: "notification_important",
          label: "Demandes rdv",
        },
        {
          path: "/accueil/file-attente",
          icon: "reorder",
    ...
    },
    {
      title: "Analyse",
      items: [
        { path: "/accueil/stats", icon: "monitoring", label: "Statistiques" },
      ],
    },
    {
      title: "Compte",
      items: [
        { path: "/accueil/profil", icon: "person", label: "Mon Profil" },
        {
          path: "/accueil/notifications",
          icon: "notifications",
          label: "Notifications",
        },
        { path: "/accueil/aide", icon: "help", label: "Aide" },
      ],
    },
  ];
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50 dark:bg-slate-950 transition-colors duration-[800ms]">
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 bg-white dark:bg-[#1c2229] border-r border-slate-200 dark:border-[#2d363f] flex flex-col p-4 shrink-0 transition-all duration-[400ms] ease-in-out transform ${
          isSidebarOpen
            ? "w-72 translate-x-0"
      ...
      >
        <div className="flex flex-col h-full overflow-hidden">
          {/* Logo Section */}
          <div
            className={`mb-8 transition-all duration-300 ${isSidebarOpen ? "px-2" : "flex justify-center"}`}
          >
            ...
            {menuSections.map((section, idx) => (
              <div key={idx} className="flex flex-col gap-1.5">
                ...
                {section.items.map((item) => (
                  ...
                  >
                    <span
                      className={`material-symbols-outlined text-[22px] shrink-0 transition-colors duration-200 ${
                        isActive(item.path)
                          ? ""
          ...
          <div className="mt-auto pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-col gap-4">
            ...
            <button
              ...
              className={`flex items-center rounded-xl h-11 bg-slate-50 dark:bg-slate-800 text-titles dark:text-white text-[13px] font-bold transition-all duration-200 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 ${
                isSidebarOpen ? "px-4 gap-2 w-full" : "justify-center w-full"
              }`}
      ...
      {/* Main Content */}
      <main
        className={`flex-1 flex flex-col overflow-y-auto relative transition-all duration-[400ms] ${isSidebarOpen ? "lg:pl-72" : "lg:pl-20"}`}
      >
        {/* Header */}
            ...
            <div className="flex items-center gap-6 flex-1">
              ...
              <div className="flex-1 max-w-2xl hidden sm:block">
                <label className="flex items-center w-full h-11 bg-slate-50 dark:bg-slate-800 rounded-xl px-4 group ring-1 ring-slate-200 dark:ring-slate-700 focus-within:ring-2 focus-within:ring-primary/40 transition-all duration-[400ms]">
                  <span className="material-symbols-outlined text-slate-400 mr-3 text-[22px]">
                    search
                  </span>
                  <input
                ...
                {isProfileOpen && (
                  <div className="absolute top-[calc(100%+12px)] right-0 w-72 bg-white dark:bg-[#1c2229] border border-slate-200 dark:border-[#2d363f] rounded-2xl shadow-2xl shadow-black/10 z-[60] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                    <div className="p-5 border-b border-slate-100 dark:border-[#2d363f] bg-slate-50/50 dark:bg-[#252c35]/50">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          ...
                          <p className="text-[11px] text-[#6c757f] font-medium">
                            Accueil
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="p-2">
                      <Link
                        to="/accueil/profil"
                        className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-[#252c35] group transition-all"
                      ...
                      >
                        <div className="size-10 rounded-lg bg-slate-100 dark:bg-[#15191d] group-hover:bg-primary/10 flex items-center justify-center">
                          <span className="material-symbols-outlined text-[20px] text-slate-500 group-hover:text-primary">
                            person
                          </span>
                        </div>
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] text-[#6c757f]">
